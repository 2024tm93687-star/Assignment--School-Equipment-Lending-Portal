...
// Ensure we always have /api/auth in the path
const getAuthServiceUrl = () => {
  const baseUrl = AUTH_SERVICE_URL.endsWith("/") ? AUTH_SERVICE_URL.slice(0, -1) : AUTH_SERVICE_URL;
...
const authenticate = async (req, res, next) => {
  try {
    ...
    const response = await fetch(`${getAuthServiceUrl()}/validate`, {
      method: "GET",
      headers: {
    ...
    // Attach user data returned by auth service (username, role, userId)
    req.user = userData;
